...
/*   Updated: 2024/03/13 15:36:13 by wiferrei         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool	check_env_var_format(char *data)
{
	...
	{
		status_error(data, "Syntax error: malformed environment variable", STDERR_FILENO);
		shell()->status = 258;
		return (false);
	}
	p = data + 1;
...
bool	grammar_env_var(t_parser *parser)
{
	if (!parser->tokens)
	{
		status_error("", "Syntax error: unexpected end of input after env var", STDERR_FILENO);
		shell()->status = 258;
		return (false);
	}
